...
node['ldm']['build_dependencies'].each do |pkg|
  package pkg
end
...
remote_file ldm_cache_file do
  ...
  checksum node['ldm']['checksum']
  notifies :run, "bash[extract_ldm]"
end
...
bash 'build_ldm' do
  ...
  code <<-EOH
    ...
    make install
  EOH
  notifies :run, "bash[run_ldm_root_actions]"
end
